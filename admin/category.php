...
$page = 'gallery';
if (isset($_GET['del'])) {
    $gallery_category_id = $_GET['del'];
    $to_delete = mysql_query("SELECT * FROM gallery_category WHERE gallery_category_id='$gallery_category_id' ") or die(mysql_error());
    $row = mysql_fetch_object($to_delete);
    $name = $row->gallery_name;

    $delete = mysql_query("DELETE FROM gallery_category WHERE gallery_category_id = $gallery_category_id") or die(mysql_error());
    if ($delete) {
        store_log($user_id, $user_fullname.' deleted category '.$name);
    }
...
if (isset($_POST['edit_category'])) {
    ...
    $insert = mysql_query("UPDATE gallery_category SET gallery_name='$gallery_name', gallery_url='$gallery_url' WHERE gallery_category_id=$gallery_category_id") or die(mysql_error());

    if ($insert) {
        store_log($user_id, $user_fullname.' edited category '.$gallery_name);
        $message = 'Category has been Edited.';
    }
    else
    {
        $error = 'Cateogory didn\'t Edit Try again!!!';
    }
}
if(isset($_POST['addcategory']))
    {
        ...
        if ($result) {
        store_log($user_id, $user_fullname.' added category '.$gallery_name);
        $message = 'Category has been added.';
    }
    else
    {
        $error = 'Cateogory didn\'t Add Try again!!!';
    }
    }
?>
<!DOCTYPE html>
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="ibox bg-boxshadow mb-30">
                                    <div class="ibox-title">
                                        <h5 class="mb-30">Category<button class="btn btn-primary f-right" data-toggle="modal" data-target="#addMenu">Add <i class="fa fa-plus"></i></button></h5>
                                    </div>
                                    <!-- Modal Primary -->
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-striped table-bordered table-hover dataTables-example">
                                                ...
                                                <tbody>
                                                    <?php
                                                        $i =1;
                                                        $sql = mysql_query("SELECT * FROM gallery_category ORDER BY gallery_category_id DESC") or die(mysql_error());
                                                        while ($row = mysql_fetch_object($sql)) {
                                                    ?>
                                                        <tr class="grade">
                                                            <td><?php print $i++; ?></td>
                                                            <td><?php print $row->gallery_name; ?></td>
                                                            <td class="center"> 
                                                                <a class="popup_edit" id="<?php print $row->gallery_category_id; ?>"> <i class="fa fa-pencil"></i> </a> 
                                                                   <a onclick="confirm_delete(<?php print $row->gallery_category_id; ?>)"> <i class="fa fa-trash"></i> </a> 
                                                            </td>
                                                        </tr>
                                                                <?php
                                                            }
                    ...
                    <div class="panel-body">
                        <form action="" class="form-horizontal form-bordered" method="post">
                                ...
                                <div class="col-md-10">
                                    <section class="form-group-vertical">
                                        <div class="input-group input-group-icon">
                                            <input class="form-control" name="gallery_name" type="text" placeholder="Name" required="">
                                        </div>
                                    </section>
                                </div>
                            </div>
                            <div class="form-group">
                                ...
                                <div class="col-md-6">
                                        <div class="input-group input-group-icon">
                                            <button type="submit" name="addcategory" class="mb-xs mt-xs mr-xs btn btn-primary"><i class="fa fa-image"></i> Add Category</button>
                                        </div>
                                </div>
                            </div>
